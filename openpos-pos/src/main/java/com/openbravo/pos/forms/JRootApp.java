...
import com.openbravo.pos.legacy.forms.StandardBottomPanelCreator;
 ...
 * @author adrianromero
 */
public class JRootApp extends JPanel implements AppView, OnStatusChangedNotifier, OnInitializedNotifier {
	...
	public boolean initApp() {
		...
		if (!AppLocal.DB_VERSION.equals(sDBVersion)) {
			...
			String sScript = sDBVersion == null ? m_dlSystem.getInitScript() + "-create.sql" : m_dlSystem
					.getInitScript() + "-upgrade-" + sDBVersion + ".sql";

			if (JRootApp.class.getResource(sScript) == null) {
				JMessageDialog.showMessage(
						this,
						new MessageInf(MessageInf.SGN_DANGER, sDBVersion == null ? AppLocal.getIntString(
								"message.databasenotsupported", session.DB.getName()) // Create script does not exists. Database not supported
		...
		{
			SimpleAppContext appContext = new SimpleAppContext(this, m_props, m_dlSystem, session);
			appContext.addBean(m_sInventoryLocation, "inventoryLocation");
			onInitializedListener.onInitialized(appContext);
		}
	...
	private void openAppView(AppUser user) {
		...
		if (closeAppView()) {
			...
			if (onStatusChangedListener != null)
				onStatusChangedListener.onAppOpen(new SimpleAppContext(this, m_principalapp));
	...
	public boolean closeAppView() {
		...
		if (m_principalapp == null) {
			...
			// The user status notificator
			if (onStatusChangedListener != null)
				onStatusChangedListener.onAppClose(new SimpleAppContext(this, m_principalapp));
	...
	private void initComponents() {
		...
		m_jPanelTitle.setBorder(javax.swing.BorderFactory.createMatteBorder(0, 0, 1, 0, javax.swing.UIManager
		...
		add(createBottomPanel(), java.awt.BorderLayout.SOUTH);
	}// </editor-fold>//GEN-END:initComponents

	private JPanel createBottomPanel() {

		String bottomPanelCreatorClass = m_props.getProperty("bottom.panel.creator");
	
		if (bottomPanelCreatorClass == null) {
			return new StandardBottomPanelCreator().createBottomPanel(new SimpleAppContext(this,m_props));
		}
		else {
			try {
				Class<BottomPanelCreator> bottomPanelCreator = (Class<BottomPanelCreator>)Class.forName(bottomPanelCreatorClass);
				return bottomPanelCreator.newInstance().createBottomPanel(new SimpleAppContext(this, m_props));
			}
			catch (Exception e) {
